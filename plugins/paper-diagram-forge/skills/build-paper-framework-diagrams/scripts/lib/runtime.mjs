...
export async function loadPackage(packageName) {
  ...
  for (const root of moduleRoots()) {
    ...
    try {
      const resolver = createRequire(path.join(root, "paper-diagram-forge-resolver.cjs"));
  ...
  throw new Error(
    `Unable to load ${packageName}. In Codex Desktop, call load_workspace_dependencies and set PAPER_DIAGRAM_FORGE_NODE_MODULES to its Node.js packages path. ${failures.join(" | ")}`,
  );
}
