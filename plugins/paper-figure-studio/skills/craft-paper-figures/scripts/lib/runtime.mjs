...
function moduleRoots() {
  const roots = [];
  for (const value of [
    process.env.PAPER_FIGURE_STUDIO_NODE_MODULES,
...
export async function loadPackage(packageName) {
  ...
  for (const root of moduleRoots()) {
    ...
    try {
      const resolver = createRequire(path.join(root, "paper-figure-studio-resolver.cjs"));
  ...
  throw new Error(
    `Unable to load ${packageName}. In Codex Desktop, call load_workspace_dependencies and set PAPER_FIGURE_STUDIO_NODE_MODULES to its Node.js packages path. ${failures.join(" | ")}`,
  );
}
